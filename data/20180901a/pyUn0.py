...
class us_spi:
    ...
    def set_timings(self, t1, t2, t3, WaitTill, t5):
        ...
        if self.Nacq > 499999:
            raise NameError(
                'Acquisition length over 500.000 points (8Mb = Flash limit)')
        ...
        if len(tgc_values) < 43:  # to correct
            for i in range(len(tgc_values)):
                if (tgc_values[i] >= 0) and (tgc_values[i] < 1020):
                    ...
                    self.write_fpga(16+i, tgc_values[i]/4)
                else:
    ...
    def loop_acq(self):
        ...
        while 1:
            ...
            # Software Trig : As to be clear by software
            self.write_fpga(0xEA, 0x0)
    ...
    def do_acquisition(self):
        ...
        if gpioexists:
            ...
            name_json = self.JSON["experiment"]["id"] + \
                "-"+str(self.JSON["N"])+".json"
    ...
    def config_spi(self):
        ...
        # How many cycles in countinious mode
        self.set_acquisition_number_lines(0xA0)
    ...
    def set_delta_acq(self, acquisition_delay_val):
        ...
        print("Delay between:", hDA*1000/128,
              "ns -- ", hex(hDAMSB), hex(hDALSB))
    ...
    def set_length_acq(self, LAcqI):
        ...
        self.JSON["parameters"]["LengthAcq_Real"] = int(
            correct_length_acq*1000/128)
        length_acq_msb = 0x00FF & correct_length_acq/256
        length_acq_lsb = 0x00FF & correct_length_acq
        if self.verbose:
            print("Acquisition length: ", int(
                correct_length_acq*1000/128), "ns.")
    ...
    def set_period_between_acqs(self, lEPeriod):
        ...
        print("Arguments:", hex(repeat_length_msb), hex(
            repeat_length), hex(repeat_length_lsb))
        self.JSON["parameters"]["PeriodAcq"] = int(lEPeriod)
        self.JSON["parameters"]["PeriodAcq_Real"] = int(
            repeat_length_arg*1000/128)
    ...
    if pyexivexists:
        ...
        for file_name in Imgs:
            ...
            try:
                ...
                # Modules
                # "matty, cletus"
                metadata['Exif.Image.Software'] = list_modules
                metadata['Exif.Image.Make'] = exp_id  # "20180516a"
                # "oscilloscope"
                metadata['Exif.Photo.MakerNote'] = img_category
                # "Unpacking data"
...
class us_json:
        ...
        with open(path) as json_data:
            ...
            for i in range(nLp):
                if (A[2*i+1]) < 128:
                    ...
                    IDLine.append(((A[2*i+0] & 0b11110000)/16 - 8) / 2)
                    ...
                    # Identify the # of the line
                    IDLine.append(((A[2*i+1] & 0b11110000)/16 - 8) / 2)
            ...
            t = [1.0*x/self.f + self.timings['t4'] /
                 1000.0 for x in range(len(tmp))]
            self.t = t
            ...
            self.processed = True
            self.Duration = (
                self.parameters['LengthAcq']-self.parameters['DeltaAcq'])/1000.0
    ...
    def create_fft(self):
        ...
        if self.processed:
            ...
            description_experiment += " experiment. " + \
                self.experiment["description"]
            tag_image(file_name, "matty, cletus", self.iD,
                      "FFT", description_experiment)
    ...
    def mkImg(self):
        ...
        if self.processed:  # @todo check this to get env & al
            ...
            if self.show_images:
                plt.show()
            tag_image(file_name, "matty", self.iD, "graph", "Automated image of " +
                      self.iD + " experiment. "+self.experiment["description"])
    ...
    def mk2DArray(self):
        ...
        y = [s for s in img if (
            len(s) > duration_self-10 and len(s) < duration_self+10)]
        L = len(y)
        # for k in range(L):
        #    L = max(L,len(y[k]))
        ...
        Duration = (self.parameters['LengthAcq'] -
                    self.parameters['DeltaAcq'])/1000.0

        clean_image = clean_image[:, :int(Duration*self.f)]
        plt.figure(figsize=(15, 10))
        if self.Nacq > 1:
            print(img_size[1], img_size[0])
            plt.imshow(np.sqrt(np.abs(clean_image)), cmap='gray',
                       aspect=0.5*(img_size[1]/img_size[0]), interpolation='nearest')
        else:
        ...
        plt.savefig(file_name)
        tag_image(file_name, "matty, "+self.piezo, self.iD, "BC",
    ...
    def save_npz(self):
        ...
        """
        import pickle
        # +".npz" # @todo: create folder if not.
        path_npz = "data/"+self.iD+"-"+str(self.N)
    ...
    def plot_detail(self, nb_line, Start, Stop):  # @todo: use it when processing data
        ...
        plot_time_series = self.t[Offset +
                                  int(Start/self.f):Offset+int(Stop*self.f)]
        ...
        plot_title = "Detail of "+self.iD + \
            " - acq. #: " + str(self.N)+", between "
    ...
    def mkSpectrum(self, img):
        ...
        if len(img):
            ...
            # @usuned, why?
            self.FFT_x = [X*self.f / len_lines for X in range(len_lines)]
            for k in range(n_lines):
                fft_single_line = np.fft.fft(img[k])
                Spectrum.append(fft_single_line[0:n_lines/2])

            plt.figure(figsize=(15, 10))
            plt.imshow(np.sqrt(np.abs(Spectrum)), extent=[
                       0, 1000.0*self.f/2, n_lines, 0], cmap='hsv', aspect=30.0, interpolation='nearest')
    ...
    def create_title_text(self):
        ...
        """
        title_text = "Experiment: " + self.iD+"-" + \
            str(self.N)+"\nDuration: "+str(self.Duration)
        title_text += "us ("+str(self.parameters['LengthAcq'])+" - "
        title_text += str(self.parameters['DeltaAcq'])+"), for "+str(self.Nacq)
        title_text += " repeats "
        title_text += "each " + \
            str(self.parameters['PeriodAcq_Real']/1000.0)+"us\n"
        title_text += "Fech = " + \
            str(self.f)+"Msps, total of "+str(float(self.f)*self.Duration)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if "single" in sys.argv[1]:
            ...
            TGCC = UN0RICK.create_tgc_curve(10, 980, True)[0]
            # We then apply the curve
            UN0RICK.set_tgc_curve(TGCC)
            # Setting 2.5ms between shots
            UN0RICK.set_period_between_acqs(int(2500000))
            UN0RICK.JSON["N"] = 1 				 # Experiment ID of the day
            # Single acquisition
            UN0RICK.set_multi_lines(False)
            # Setting the number of lines (1)
            UN0RICK.set_acquisition_number_lines(1)
            # Sampling speed setting
            UN0RICK.set_msps(0)
            # Settings the series of pulses
            A = UN0RICK.set_timings(200, 100, 2000, 5000, 200000)
            # Doing the acquisition and saves
            UN0RICK.JSON["data"] = UN0RICK.do_acquisition()
        ...
        if "multi" in sys.argv[1]:
            ...
            TGCC = UN0RICK.create_tgc_curve(600, 900, False)[
                0]  # Gain: expo, 300mV to 900mv
            # We then apply the curve
            UN0RICK.set_tgc_curve(TGCC)
            # Setting 2.5ms between lines
            UN0RICK.set_period_between_acqs(int(2500000))
            UN0RICK.set_multi_lines(True)			 # Multi lines acquisition
            # Setting the number of lines (3)
            UN0RICK.set_acquisition_number_lines(10)
            # Sampling speed setting
            UN0RICK.set_msps(0)
            # Settings the series of pulses
            A = UN0RICK.set_timings(200, 100, 1000, 2000, 100000)
            # Doing the acquisition and saves
            UN0RICK.JSON["data"] = UN0RICK.do_acquisition()
        ...
        if "loop" in sys.argv[1]:
            UN0RICK = us_spi()
            UN0RICK.init()
            # Multi lines acquisition
            UN0RICK.set_multi_lines(True)
            UN0RICK.set_acquisition_number_lines(
                2)		 # Setting the number of lines
            # Acquisition Freq
            UN0RICK.set_msps(3)
            # Looping on triggers
            A = UN0RICK.set_timings(200, 100, 2000, 5000, 200000)
